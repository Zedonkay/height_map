...
def plot_mesh(file_name):
    """
    Plot the mesh generated by Kriging.
    
    Parameters:
    file_name (str): The name of the CSV file containing the interpolated grid.
    """
    try:
        print("Plotting the mesh...")
        grid_df = pd.read_csv(file_name)
        gridx = grid_df.columns.values.astype(float)
        gridy = grid_df.index.values.astype(float)
        z_pred = grid_df.values
        plt.figure(figsize=(10, 8))
        plt.pcolormesh(gridx, gridy, z_pred, shading='auto')
        ...
        print("Error plotting the mesh:", e)
        with open('error_log.txt', 'a') as f:
            f.write("Error plotting the mesh: " + str(e) + "\n")
